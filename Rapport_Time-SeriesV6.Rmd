...
* $m_t$ : La tendance (orientation à long terme)
* $s_t$ : La saisonnalité (phénomène, composante périodique ou saisonnière)
* $z_t$ : L'erreur ou résidu, dont la variation doit être faible par rapport aux 2 autres.
...
---------------------------- | -------------- | ----------- | ---------- |  
Lissage Simple (A,N,N)       | `r fitLES$aic` | fitLES$aicc | fitLES$bic | 
Lissage Double (A,A,N)       | `r fitLED$aic` | fitLED$aicc | fitLED$bic | 
Holt-Winters   (A,A,A)       | `r fitHW$aic`  | fitHW$aicc  | fitHW$bic  | 
...
Modèle   |       AIC      |      AICc   |      BIC   | 
-------- | -------------- | ----------- | ---------- |  
(M,A,A)  | `r fitMAA$aic` | fitMAA$aicc | fitMAA$bic | 
(M,A,M)  | `r fitMAM$aic` | fitMAM$aicc | fitMAM$bic | 
...
Le modèle sélectionné est le modèle avec tendance additive et avec erreur et saisonnalité multiplicatives (M,A,M). C'est aussi ce modèle qui minimise le critère AIC, AICc et BIC. En effet, en spécifiant le critère à minimiser: AIC, AICc et BIC avec le paramètre ic="aic"/"aicc" ou bic" de la fonction ets, on obtient toujours le même modèle.
       ...
       AIC      |      AICc    |      BIC    | 
--------------- | ------------ | ----------- |  
`r fit.ets$aic` | fit.ets$aicc | fit.ets$bic | 
...
Le test de blancheur des résidus rejette nettement les modèles SARIMA(0,1,1)(0,1,1) et SARIMA(2,1,1)(1,1,1) avec une p-value de l'ordre de `r b011_011$p.value` Donc H0 est accepté <=> toutes les autocorrélation sont nulles.
...
Pour les 2 autres modèles on obtient comme p-value :  `r b111_111$p.value` pour le modèle SARIMA(1,1,1)(1,1,1)  et `r b011_011$p.value` pour le modèle SARIMA(0,1,1)(0,1,1) 
...
legend("topleft",c("Vraies valeurs","SARIMA_011-111",
                   "SARIMA_112-011", "SARIMA_211-011","SARIMA_111-011","SARIMA_111-011"),col=c("darkgreen","yellow","blue","black","red")
...
plot(Traffic_SNCF,col="darkgreen",lwd=1,ylab="Nombre de Passagers",xlab="Temps", xlim=c(1979,1981),ylim=range(c(Traffic_SNCF,predSARIMA$lower,predSARIMA$upper,predHW$lower,predHW$upper)))
points(predauto_111_011$mean,col="red",lwd=2,type='l')
