...
Pour les prédictions ont ne va s'intéresser qu'au modèle obtenu SARIMA(1,1,1)(0,1,1).
Les prédictions obtenus avec les autres modèles se révèlent être très proche de ce modèle.
...
* Comparaison du modèle SARIMA(1,1,1)(0,1,1) aux vraies valeurs

```{r echo=FALSE, fig.height=5, fig.width=15}
#par(mfrow=c(1,2))
plot(Traffic_SNCF,col="darkgreen",lwd=2,ylab="Nombre de Passagers",xlab="Temps", xlim=c(1962,1982),ylim=range(c(Traffic_SNCF,predauto_63$lower,predauto_63$upper,predHW$lower,predHW$upper)))
#points(predauto_70$mean,col="blue",lwd=2,type='l')
points(predauto_63$mean,col="red",lwd=3,type='l',lty=2)

legend("topleft",
       c("Vraies valeurs","SARIMA(1,1,1)(0,1,1)"),
       col=c("darkgreen", "red"),
       lty=1,lwd=2,cex=1)
```
...
plot(Traffic_SNCF,col="darkgreen",lwd=2,ylab="Nombre de Passagers",xlab="Temps",lty=1,main="Comparaison des vraies valeurs et du modèle obtenu SARIMA(1,1,1)(0,1,1)", xlim=c(1980,1981),ylim=c(2500,4500))
#points(predauto_70$mean,col="blue",lwd=2,type='l')
...
points(predauto_63$lower[,1],col="black",type='l',lty=2)
points(predauto_63$upper[,1],col="black",type='l',lty=2)
points(predauto_63$lower[,2],col="blue",type='l',lty=2)
points(predauto_63$upper[,2],col="blue",type='l',lty=2)

legend("bottomright",
       c("Vraies valeurs","SARIMA(1,1,1)(0,1,1)","IC 80%","IC 95%" ),
       col=c("darkgreen", "red","blue","black"),
       lty=c(1,1,2,2),lwd=2,cex=1)
```

Le modèle suit plutôt bien la courbe des données des vraies valeurs, avec cependant un décalage ou retard.
On remarque aussi une amplitude du modèle qui semble plus importante.
...
plot(Traffic_SNCF,col="darkgreen",lwd=2,ylab="Nombre de Passagers",xlab="Temps",main="Comparaison du modèle obtenu SARIMA(1,1,1)(0,1,1) et du lissage exponentiel triple MAM (Holt-Winters)", xlim=c(1979,1981),ylim=range(c(2500,4500,predHW$lower,predHW$upper)))
#points(predauto_70$mean,col="blue",lwd=2,type='l')
#points(predauto_70$lower[,2],col="blue",type='l',lty=2)
#points(predauto_70$upper[,2],col="blue",type='l',lty=2)
...
legend("topleft",c("Vraies valeurs","SARIMA(1,1,1)(0,1,1)","Liss.exp","IC à 80% SARIMA(1,1,1)(0,1,1)","IC à 80% Liss-exp"),col=c("darkgreen","red","black","red","black"),lty=c(1,1,1,2,2),lwd=2,cex=1)
```

Le modèle SARIMA et le lissage exponentiel triple de type (MAM) ont des courbes très semblables. Avec un léger mieux pour le modèle SARIMA.
L'intervalle de confiance pour le modèle SARIMA est aussi plus resserré sur les vraies valeurs. Le comportement est plutôt bon, avec cependant un décalage, retard par rapport aux vraies valeurs. Peut-être due au changement (de régime) qui semble intervenir au début de l'année 1980. Sinon la forme de la courbe étant quant à elle très bien rendu. 
